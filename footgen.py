...
def pad(x1, y1, x2, y2, width, clear, mask, pinname, shape):
    if shape=="round":
        ...
        flags = "square"
    return "\tPad[%dnm %dnm %dnm %dnm %dnm %dnm %dnm \"%s\" \"%s\" \"%s\"]\n"\
...
def bga(attrlist):
    ...
    lefthand = findattr(attrlist, "lefthand")
...
def qfp(attrlist):
    ...
    if silkstyle == "inside":
        ...
        qfpelt = qfpelt + silk(-x,-y,-(x+1000000),-(y+1000000),silkwidth)
...
def so(attrlist):
    ...
    if(silkstyle == "inside"):
        ...
        soelt = soelt + silk(0,-silky+1000000,-1000000,-silky,silkwidth)
        soelt = soelt + silk(0,-silky+1000000,1000000,-silky,silkwidth)
    return soelt+")\n"
...
# convert values from source units into nanometers
def str2pcbunits(dist):
    ...
    if dist[number.end():].find("mm")!=-1:
        return int(0.5 + 1000000 * val)
    elif dist[number.end():].find("mil")!=-1:
        return int(0.5 + val * 25.4 * 1000)
    elif dist[number.end():].find("in")!=-1:
        return int(0.5 + val * 25.4 * 1000000)
    return int(0.5 + val)
...
while 1:
    ...
    if attribute[0] == "part":
        ...
        refdesx = findattr(attributes, "refdesx")
        refdesy = findattr(attributes, "refdesy")
        output_file.write("Element[\"\" \"%s\" \"Name\" \"Val\" 1000 1000 %dnm %dnm 0 100 \"\"]\n" % (part, refdesx, -1000000 - refdesy))
